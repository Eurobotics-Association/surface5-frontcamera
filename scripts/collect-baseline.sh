...
run 'module list' bash -c "lsmod | rg -i 'ipu|cio2|imgu|ov5693|ov8865|ov7251|dw9719|int3472|v4l|videodev|mc' || true"
run 'module aliases' bash -c 'for m in ipu3_cio2 ipu3_imgu ov5693 dw9719; do echo "[$m]"; modinfo "$m" 2>&1 | rg "^(filename|firmware|alias|version|srcversion):" || true; done'
...
run 'relevant packages' bash -c "dpkg-query -W -f='\${binary:Package}\t\${Version}\t\${Status}\n' 2>/dev/null | rg -i 'linux-(image|modules|headers|surface)|libcamera|pipewire|wireplumber|v4l|media' | sort || true"
